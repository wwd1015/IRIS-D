...
def _indicator_card(ind: dict) -> html.Div:
    ...
    if ind.get("note"):
        body.append(html.Div(ind["note"], className="mkt-ind-note"))
    body.append(html.Div(ind.get("threshold_text", ""), className="mkt-ind-thr"))
    body.append(html.Div([
        html.Div([src, html.Span(f"· {_fmt_time(ind.get('last_updated', ''))}"), *tags],
                 className="mkt-ind-src"),
    ], className="mkt-ind-foot"))

    return html.Div(body, className=f"mkt-ind {status}")
